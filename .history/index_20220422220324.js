...
app.get('/Fish', async (req, res) => {
    ...
    content = {fact: content[chosen]['Physical Description'].replaceAll(/<.+?>>/, " ").replaceAll('\n', ""), image: content[chosen]['Image Gallery'][0].src}
   let img = await fetch(content.img)
   img = await img.buffer()
   res.render('Fish', {fact: content.fact, buffer: img})
})